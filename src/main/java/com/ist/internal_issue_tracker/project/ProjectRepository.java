...
interface ProjectRepository extends JpaRepository<Project, Integer> {
  ...
  /**
   * Deliberately blind to {@code isActive}: {@code projects.name} carries a plain UNIQUE
   * constraint, not a partial one, so a soft-deleted project still owns its name and the pre-check
   * has to agree with what the database will do.
   */
  ...
  /**
   * Soft-deleted projects are excluded unconditionally - see {@code
   * TeamRepository#findAllByFilters} for the reasoning. Note that this is {@code isActive}, the
   * delete flag, and has nothing to do with {@code status}: a CANCELLED project is still listed,
   * because it still exists.
   *
   * <p>{@code endDateBefore} drops open-ended projects, since a null {@code end_date} cannot be
   * shown to fall before anything.
   *
   * <p>The {@code CAST}s are load-bearing. A bare {@code :param IS NULL} gives PostgreSQL nothing
   * to resolve the placeholder's type from, and it rejects the statement with {@code could not
