 ...
 * caller wanting one headline number sums the list itself; the breakdown is kept so the UI can still
 * show which project a number came from.
 *
 * <p>{@code recentAveragePoints} is null when the person has not finished a single sprint yet - see
 * {@code UserWorkService#getSprintProgress}. Null there means "no answer", not "zero"; a caller that
 * coalesces it would report a newcomer as averaging nothing, which is a different claim.
 */
