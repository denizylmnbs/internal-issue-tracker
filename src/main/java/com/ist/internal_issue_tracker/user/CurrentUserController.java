...
/**
 * "Who am I", answered from the token alone. A client needs it because {@code LoginResponse} carries
 * only the access token and the token's only claim is the user's id - so without this a caller knows
 * an id and neither a name nor, more to the point, a role.
 *
 ...
 *
 * <p>No authorization rule of its own: the principal <em>is</em> the subject, so there is nothing to
 * compare it against and {@code anyRequest().authenticated()} already says everything needed.
 */
@RestController
...
public class CurrentUserController {
   ...
   * so going back for the whole row costs the one lookup that was going to happen anyway and returns
   * the same {@link UserResponse} shape as {@code GET /api/users/{id}}, rather than a second,
   * nearly-identical record a client would have to special-case.
   */
  @GetMapping("/me")
