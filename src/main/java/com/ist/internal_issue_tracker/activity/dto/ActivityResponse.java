 ...
 * {@code ISSUE}/{@code issueId} — but required on the project-wide feed ({@code GET
 * /api/projects/{id}/activities}), which unions all three tables: without them a client cannot tell
 * a project's own {@code STATUS_UPDATED} row from an issue's, or route a click on one row to the
 * right page. {@code id} alone cannot disambiguate either, since it is only unique within one
 * table.
 */
