...
/**
 * Bugs against everything else, in one bucket, from both ends: how many arrived and how much shipped.
 *
 ...
 *
 * <p><em>Defect density</em> ({@code defectsPerCompletedIssue}, {@code defectsPerCompletedPoint}) is
 * bugs filed against work delivered, which is the ratio that actually tracks quality - it asks how
 * many defects each unit of shipped work brought with it. Both variants are returned because
 ...
 *
 * <p>Bugs are counted where they were <em>filed</em>, not where they were fixed. A defect belongs to
 * the period that produced it, and attributing it to the sprint that cleaned it up would reward
 * leaving bugs unfixed.
 *
 * <p>Every ratio is null rather than zero when its denominator is empty - see {@code DurationStats}.
 */
