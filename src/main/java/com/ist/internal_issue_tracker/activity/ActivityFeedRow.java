...
/**
 * Row shape of the project-wide activity feed's union query
 * ({@link ProjectActivityRepository#findFeedByProjectId}). An interface projection because the query
 * is native — the same reason {@code project.ProjectParticipant} and {@code project.UserProject}
 * are.
 *
 ...
 * proxy only converts between an interface and its target - not between two concrete temporal types.
 * {@code ActivityMapper} converts to {@code OffsetDateTime} when building {@code ActivityResponse}.
 */
