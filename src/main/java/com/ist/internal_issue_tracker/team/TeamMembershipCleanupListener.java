 ...
 *
 * <p>Plain {@code @EventListener} rather than {@code @ApplicationModuleListener}: the latter is
 * asynchronous and would leave a window in which a deleted team's roster is still being served. The
 * listener runs inline in the publisher's transaction, so the delete and the cleanup commit together
 * or not at all.
 */
@Component
