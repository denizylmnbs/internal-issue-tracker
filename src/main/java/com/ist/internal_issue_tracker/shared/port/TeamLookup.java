...
public interface TeamLookup {
  ...
  /**
   * The teams a user is currently on. Exists so that {@code project} can resolve the team route
   * into a project - a user is a participant if a team they are on is assigned to it - without
   * reading {@code team_users} itself. The membership table is {@code team}'s, and a query naming
   * it from another module is a boundary violation no verification can catch, because a query is a
   * string.
   *
   ...
   * no gain. Empty when the user is on no team - callers must handle that themselves, since an
   * empty {@code IN} list is not valid SQL.
   */
   ...
   * than an error for a team with no members or one that does not exist; callers here only ever
