 ...
 * the alternative would be four files that are otherwise identical. {@code subjectId} is the user
 * or the team being added or removed - never the actor, who is whoever is doing the adding.
 *
 * <p>These are not {@link ProjectChangedEvent} changes: membership has no old value to move from,
 * so a field-change shape would have to leave one side permanently null and say nothing the action
