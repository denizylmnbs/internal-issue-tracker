 ...
 * dead project's memberships are retired before anyone can read them; this one is consumed after
 * the commit, on another thread, so that a fault in the audit path cannot fail the delete. One
 * event cannot be delivered both ways, and collapsing them would mean giving up one of the two
 * guarantees.
 */
