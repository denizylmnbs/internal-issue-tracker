 ...
 * moving an issue to the next sprint would erase it from the previous sprint's burndown as though it
 * had never been committed. The number would change without the history changing, which is the one
 ...
 *
 * <p>Every field is nullable, and each null means the issue genuinely had nothing there: no type set,
 * no estimate agreed, no sprint yet. {@code priority} is the exception in practice - {@code issues}
 * declares it {@code NOT NULL} with a default - but it is left nullable here so that the record does
 * not encode a constraint belonging to another module.
 *
 ...
 */
public record IssueDimensions(
    String type, String priority, Integer storyPoint, Integer sprintId) {}
