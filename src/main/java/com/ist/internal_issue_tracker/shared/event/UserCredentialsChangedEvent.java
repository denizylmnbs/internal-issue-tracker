...
/**
 * Published whenever a user's password is changed or reset. {@code auth} listens for this to
 * revoke every refresh token the user currently holds, so a session issued before the change
 * cannot keep renewing itself past it - the same reasoning {@link UserDeactivatedEvent} follows for
