 ...
 *
 * <p><b>Delivered asynchronously</b>, unlike the {@code *DeactivatedEvent} records next to it.
 * Those are consumed inside the publisher's transaction because a stale roster must never be
 * readable for even an instant; an activity row has no such window - nothing reads it to make a
 * decision. Being asynchronous is what keeps a fault in the audit path from turning a successful
 * issue write into a 500, and Spring Modulith's {@code event_publication} registry is what keeps
 * the event from being lost in exchange.
 *
 ...
 * to do with when the change happened. Every metric built on this log is a difference between two
 * of these timestamps, so a consumer-side clock reading would feed queue latency straight into
 * cycle time - and, after a restart replays outstanding publications, could be off by hours.
 *
 ...
 * {@link IssueDimensions}. On this event they describe the issue as it was filed, which is what
 * makes "how many bugs were opened this week" answerable without reading {@code issues}.
 */
