...
/**
 * Runs inside the security filter chain - an authenticated-but-unauthorized request never reaches
 * {@code GlobalExceptionHandler}, so the {@code ApiResponse} envelope has to be written here
 * directly (mirrors {@link RestAuthenticationEntryPoint}).
 */
@Component
