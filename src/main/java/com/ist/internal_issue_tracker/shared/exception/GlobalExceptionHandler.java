 ...
 * <p>
 * Extends {@link ResponseEntityExceptionHandler} rather than being a bare
 * {@code @RestControllerAdvice}: the base class already handles ~20 Spring MVC
 * exceptions (wrong HTTP method, unsupported media type, malformed JSON,
 * unmatched route, ...) with the correct status and headers (e.g. {@code Allow}
 * on 405). A bare advice would let every one of those fall through to Boot's
 * default {@code BasicErrorController} shape, silently breaking the single
 * response envelope this class exists to guarantee.
 * <p>
 * Not covered here: exceptions raised in the security filter chain
 * (before {@code DispatcherServlet}), e.g. an unauthenticated request hitting
 * a protected endpoint. Those never reach a {@code @RestControllerAdvice} and
 * must be wired via a custom {@code AuthenticationEntryPoint} /
 * {@code AccessDeniedHandler} once {@code SecurityConfig} exists.
 */
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    private ApiError fromProblemDetail(ProblemDetail pd, HttpStatusCode statusCode, WebRequest request) {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(
        ...
        List<ApiError.FieldError> fieldErrors = ex.getBindingResult().getFieldErrors().stream()
                .map(fe -> new ApiError.FieldError(fe.getField(), safeMessage(fe.getDefaultMessage(), fe.getCode())))
                .toList();
        List<ApiError.FieldError> globalErrors = ex.getBindingResult().getGlobalErrors().stream()
                .map(oe -> new ApiError.FieldError(oe.getObjectName(), safeMessage(oe.getDefaultMessage(), oe.getCode())))
                .toList();

        List<ApiError.FieldError> merged = Stream.concat(fieldErrors.stream(), globalErrors.stream())
        ...
        ApiError error = ApiError.validation(CommonErrorCode.VALIDATION_FAILED.defaultMessage(), path(request), merged);
        return handleExceptionInternal(ex, ApiResponse.error(error), headers, HttpStatus.BAD_REQUEST, request);
    }
    ...
    protected ResponseEntity<Object> handleHandlerMethodValidationException(
        ...
        ApiError error = ApiError.validation(CommonErrorCode.VALIDATION_FAILED.defaultMessage(), path(request), fieldErrors);
        return handleExceptionInternal(ex, ApiResponse.error(error), headers, HttpStatus.BAD_REQUEST, request);
    }
    ...
    protected ResponseEntity<Object> handleTypeMismatch(
        ...
        String message = "'" + ex.getValue() + "' is not a valid value for '" + property + "' (expected " + requiredType + ")";
    ...
    ResponseEntity<ApiResponse<Void>> handleApp(AppException ex, WebRequest request) {
        ...
        if (status.is5xxServerError()) {
            ...
            return ResponseEntity.status(status).body(ApiResponse.error(ApiError.internal(traceId, path(request))));
        }
    ...
    ResponseEntity<ApiResponse<Void>> handleConstraintViolation(ConstraintViolationException ex, WebRequest request) {
    ...
    /**
     * Safety net for a unique-index race that slipped past a service-layer
     * pre-check. Deliberately generic in v1: mapping specific constraint names
     * (e.g. {@code unique_active_sprint_name_per_project}) to module-specific
     * codes would require shared to know module vocabulary. Services should
     * throw {@link DuplicateResourceException}/{@link ConflictException}
     * explicitly for the expected case; this only catches races.
     */
    ...
    ResponseEntity<ApiResponse<Void>> handleDataIntegrityViolation(DataIntegrityViolationException ex, WebRequest request) {
        ...
        ApiError error = ApiError.of(
                CommonErrorCode.CONFLICT.code(), CommonErrorCode.CONFLICT.defaultMessage(), path(request));
        return ResponseEntity.status(HttpStatus.CONFLICT).body(ApiResponse.error(error));
    }

    @ExceptionHandler(OptimisticLockingFailureException.class)
    ResponseEntity<ApiResponse<Void>> handleOptimisticLocking(OptimisticLockingFailureException ex, WebRequest request) {
    ...
    /**
     * Only reached for access decisions made inside dispatch (e.g. method
     * security via {@code @PreAuthorize}). {@code AccessDeniedException} thrown
     * by the filter chain never reaches this advice - see class Javadoc.
     * An explicit handler is mandatory here: without it, this exception falls
     * through to {@link #handleUnexpected}, turning a 403 into a silent 500.
     */
    @ExceptionHandler(AccessDeniedException.class)
    ResponseEntity<ApiResponse<Void>> handleAccessDenied(AccessDeniedException ex, WebRequest request) {
        log.warn("Access denied at {}", path(request));
        ApiError error = ApiError.of(
                CommonErrorCode.FORBIDDEN.code(), CommonErrorCode.FORBIDDEN.defaultMessage(), path(request));
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ApiResponse.error(error));
    }

    @ExceptionHandler(AuthenticationException.class)
    ResponseEntity<ApiResponse<Void>> handleAuthentication(AuthenticationException ex, WebRequest request) {
        log.warn("Authentication failed at {}", path(request));
        ApiError error = ApiError.of(
                CommonErrorCode.UNAUTHENTICATED.code(), CommonErrorCode.UNAUTHENTICATED.defaultMessage(), path(request));
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(ApiResponse.error(error));
    }

    /** Last resort. Never leak {@code ex.getMessage()} - it routinely contains SQL, table names, or connection strings. */
    ...
    ResponseEntity<ApiResponse<Void>> handleUnexpected(Exception ex, WebRequest request) {
        ...
        return ResponseEntity.internalServerError().body(ApiResponse.error(ApiError.internal(traceId, path(request))));
    }
